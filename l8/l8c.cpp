...
void * rsp_func(void * params)
{
	while(flag_response)
	{
		...
		if(send(sock_server, message.c_str(), message.length(), 0) > 0) cout << "Response \"" << message << "\" sent." << endl;
		else perror("send");
...
void * rcv_func(void * params)
{
	while(flag_rcv)
	{
		...
		int rcv = recv(sock_server, buf, MAX_BUF, 0);
...
{	
	...
	while(flag_waitconnect)
	{
		...
		if(connect_res  == 0)
		{
			cout << "Connected. Res: " << connect_res << endl;
...
int main()
{
	...
	if(close(sock_server) == 0) cout << "sock_client closed." << endl;
	else perror("close");
